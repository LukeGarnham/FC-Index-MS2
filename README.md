...
5.  There is only one link on my webpage.  It appears in the footer and links to the [Football API website](https://www.api-football.com/).  I have tested this and the link opens the webpage in a new window.
6.  I performed several test on the Report a Bug form:
...
7.  I have opened my website on a laptop using Google Chrome, an Android mobile and an iPhone.  The website functions well on all devices and fits the various screen sizes well.  I noticed that on Android, the map can be opened in full screen which improves user experience however this functionality isn't enabled on iPhone.
8.  I used the Lighthouse tool in the Chrome Inspect tools to test the performance of my website.  I ran the tool at the end of my project and made the following observation and changes:
    1. The report advised that the logos in my website had no explicit height or width.  In my CSS, I had applied a maximum-height to all of the images but in fact, this could be changed simply to a height definition - I made this change to the "header img" rule and ".small-img" class rule.
    2. The Lighthouse report highlighted a lack of best practice when linking to an external webpage.  [This webpage](https://web.dev/external-anchors-use-rel-noopener/?utm_source=lighthouse&utm_medium=devtools) explains the problem my site could be exposed and solutions to negate this risk.  I subsequently added "rel="noopener"" code to the anchor tag in my footer.

9. A user navigating with a keyboard rather than a mouse will only find 5 elements on the main page which they can interact with.  The 
